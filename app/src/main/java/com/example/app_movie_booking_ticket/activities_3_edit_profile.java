...
public class activities_3_edit_profile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (uid == null && mAuth.getCurrentUser() != null) uid = mAuth.getCurrentUser().getUid();
        if (uid == null) {
                        ...
                        usersRef.child(finalUid).updateChildren(defaultAvatar)
                                .addOnCompleteListener(task -> {
                                    if (task.isSuccessful()) {
                                        ...
                                        Toast.makeText(activities_3_edit_profile.this, "Đã gán avatar mặc định.", Toast.LENGTH_SHORT).show();
                                    } else {
                                        extra_sound_manager.playError(activities_3_edit_profile.this);
                                        Toast.makeText(activities_3_edit_profile.this, "Không thể cập nhật avatar mặc định: " + Objects.requireNonNull(task.getException()).getMessage(), Toast.LENGTH_LONG).show();
                                    }
                                });
    ...
    private void uploadToImgBB(Uri imageUri, String uid, Map<String, Object> updates) {
        try {
            ...
            Bitmap scaled = Bitmap.createScaledBitmap(original, Math.max(1, (int)(width * scale)), Math.max(1, (int)(height * scale)), true);

            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            scaled.compress(Bitmap.CompressFormat.JPEG, 80, baos);
            String encodedImage = Base64.encodeToString(baos.toByteArray(), Base64.NO_WRAP);

            OkHttpClient client = new OkHttpClient.Builder().callTimeout(60, java.util.concurrent.TimeUnit.SECONDS).build();
            ...
            client.newCall(request).enqueue(new Callback() {
                @Override public void onFailure(@NonNull Call call, @NonNull IOException e) {
                    runOnUiThread(() -> {
                        extra_sound_manager.playError(activities_3_edit_profile.this);
                        Toast.makeText(activities_3_edit_profile.this, "Lỗi upload ảnh: " + e.getMessage(), Toast.LENGTH_LONG).show();
                    });
                ...
                @Override public void onResponse(@NonNull Call call, @NonNull Response response) {
                    try {
                        ...
                        if (response.isSuccessful()) {
                            ...
                            usersRef.child(uid).updateChildren(updates).addOnCompleteListener(task -> runOnUiThread(() -> {
                                if (task.isSuccessful()) {
                                    extra_sound_manager.playSuccess(activities_3_edit_profile.this);
                            ...
                            runOnUiThread(() -> {
                                extra_sound_manager.playError(activities_3_edit_profile.this);
                                Toast.makeText(activities_3_edit_profile.this, "Upload thất bại (" + response.code() + "): " + respStr, Toast.LENGTH_LONG).show();
                            });
                        }
                    } catch (Exception ex) {
                        runOnUiThread(() -> {
                            extra_sound_manager.playError(activities_3_edit_profile.this);
                            Toast.makeText(activities_3_edit_profile.this, "Lỗi xử lý phản hồi upload: " + ex.getMessage(), Toast.LENGTH_LONG).show();
                        });
